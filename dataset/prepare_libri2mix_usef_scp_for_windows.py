...
path normalization and prints progress while scanning/validating large datasets
on /mnt/<drive>, where file metadata checks can be noticeably slower.
...
To generate scp files:
    python3 prepare_libri2mix_usef_scp_for_windows.py generate \
      --librimix-root /mnt/d/Datasets/Libri2Mix \
      --out-dir libri2mix_scp/ \
...
def scan_subset(
    ...
    for mix_idx, mix_path in enumerate(sorted(mix_dir.glob("*.wav")), 1):
        ...
        if mix_idx % 5000 == 0:
            print(f"[INFO] {subset}: scanned {mix_idx} mixtures ...", flush=True)
    if not items:
...
def generate_subset_scp(
    ...
    for item_idx, item in enumerate(items, 1):
        ...
        if item_idx % 5000 == 0:
            print(f"[INFO] {subset}: generated {item_idx}/{len(items)} mixtures ...", flush=True)
...
def validate_scp_dir(scp_dir: Path, check_exists: bool = False) -> Dict[str, int]:
    ...
    bad: List[str] = []
    if check_exists:
        print(f"[INFO] {scp_dir}: validating {len(mix)} keys with file-existence checks ...", flush=True)
    else:
        print(f"[INFO] {scp_dir}: validating {len(mix)} keys ...", flush=True)

    for idx, key in enumerate(mix, 1):
        parts = key.split("_")
        if len(parts) < 5:
        ...
        if idx % 20000 == 0:
            print(f"[INFO] {scp_dir}: validated {idx}/{len(mix)} keys ...", flush=True)
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    for subset in args.subsets:
        print(f"[INFO] scanning {subset} under {libri_root} ...", flush=True)
        ...
        print(f"[OK] {subset}: scanned {len(items_by_subset[subset])} mixtures", flush=True)

    print("[INFO] building auxiliary speaker index ...", flush=True)
    pool_by_subset = build_aux_index(items_by_subset=items_by_subset)

    counts: Dict[str, Dict[str, int]] = {}
    for subset, items in items_by_subset.items():
        print(f"[INFO] {subset}: generating SCP entries ...", flush=True)
    ...
    for subset in args.subsets:
        print(f"[INFO] {subset}: validating generated SCP files ...", flush=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    gen.add_argument(
        ...
        action="store_true",
        help=(
            "Check every derived s1/s2 source wav while scanning. "
            "Disabled by default in this Windows/WSL variant because /mnt/<drive> "
            "metadata checks are slow; use --check-exists for final SCP validation."
        ),
    )
