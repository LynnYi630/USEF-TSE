...
Recommended for reproducing the author's released checkpoint:
    ...
    python prepare_wsj0_usef_scp.py rewrite \
        --src-dir /home/wangyi/USEF-TSE-ZBang/data/test/wsj0-2mix \
        --out-dir /home/wangyi/usef/data/test/wsj0-2mix_usef_sepformer_original \
        --wsj0-root /home/wangyi/datasets/wsj0_8k \
        --wsj02mix-root /home/wangyi/datasets/wsj0-2mix \
        --check-exists

Then set config test path to data/test/wsj0-2mix_abs.

To generate new scp files:
	python3 prepare_wsj0_usef_scp.py generate \
  		--wsj0-root /home/wangyi/datasets/wsj0_8k \
  		--wsj02mix-root /home/wangyi/datasets/wsj0-2mix \
...
def command_rewrite(args: argparse.Namespace) -> None:
    ...
    if args.src_dir:
        ...
        mix_scp = Path(args.mix_scp).expanduser().resolve()
        ref_scp = Path(args.ref_scp).expanduser().resolve()
        aux_scp = Path(args.aux_scp).expanduser().resolve()
...
def build_wsj0_index(
    ...
    for search_root in search_roots:
        for wav in search_root.rglob("*.wav"):
            spk = wav.stem[:3]
            index.setdefault(spk, []).append(wav.resolve())
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--wsj0-root", default="/home/wangyi/datasets/wsj0")
    p.add_argument("--wsj02mix-root", default="/home/wangyi/datasets/wsj0-2mix")
